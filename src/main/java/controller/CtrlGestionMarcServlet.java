		...
		public List<Article> readCSV(InputStream fileContent) throws IOException {
			...
			try (BufferedReader reader = new BufferedReader(new InputStreamReader(fileContent))) {
				...
				while ((line = reader.readLine()) != null) {
					...
					// Assuming the order of columns in CSV matches the order of fields in Article
					// class
					int EAN = Integer.parseInt(values[0].trim());
					String vignetteArticle = values[1].trim();
					float prixUnitaireArticle = Float.parseFloat(values[2].trim());
					String NutriscoreArticle = values[3].trim();
					String libelleArticle = values[4].trim();
					float poidsArticle = Float.parseFloat(values[5].trim());
					float prixKgArticle = Float.parseFloat(values[6].trim());
